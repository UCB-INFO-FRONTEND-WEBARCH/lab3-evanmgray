...
const findLongestSessionUser = (data) => {
  // TODO: use for loop
  let longestAvgSession = 0;
  let longestUser = "N/A";
  for (const user of data) {
    if (user.avgSessionDuration > longestAvgSession) {
      longestAvgSession = user.avgSessionDuration;
...
const getTotalSessions = (data) => {
  // TODO: use reduce
  const output = data.reduce((sum, user) => {
    return sum + user.totalSessions;
  }, 0);
  return output;
